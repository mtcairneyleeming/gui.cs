...
namespace Mono.Terminal.Internal {
	...
	class UnmanagedLibrary {
		...
		static bool IsUnity, IsXamarinIOS, IsXamarinAndroid, IsXamarin;
		...
		static UnmanagedLibrary()
		{
			...
			if (!IsMono) IsNetCore = Type.GetType("System.MathF") != null;
#if GUICS
		...
		static string GetUname()
		{
			...
			try {
				if (uname(buffer) == 0) return Marshal.PtrToStringAnsi(buffer);
				return string.Empty;
			} catch {
				return string.Empty;
			} finally {
		...
		public IntPtr LoadSymbol(string symbolName)
		{
			...
			if (IsLinux) {
				if (IsMono) return Mono.dlsym(NativeLibraryHandle, symbolName);
				if (IsNetCore) return CoreCLR.dlsym(NativeLibraryHandle, symbolName);
				return Linux.dlsym(NativeLibraryHandle, symbolName);
			}

			if (IsMacOSPlatform) return MacOSX.dlsym(NativeLibraryHandle, symbolName);
		...
		{
			if (IsWindows) return Windows.LoadLibrary(libraryPath);
			if (IsLinux) {
				if (IsMono) return Mono.dlopen(libraryPath, RTLD_GLOBAL + RTLD_LAZY);
				if (IsNetCore) return CoreCLR.dlopen(libraryPath, RTLD_GLOBAL + RTLD_LAZY);
				return Linux.dlopen(libraryPath, RTLD_GLOBAL + RTLD_LAZY);
			}

			if (IsMacOSPlatform) return MacOSX.dlopen(libraryPath, RTLD_GLOBAL + RTLD_LAZY);
			throw new InvalidOperationException("Unsupported platform.");
		}
