...
namespace Terminal.Gui {
	...
	public class View : Responder, IEnumerable {
		...
		public virtual void RemoveAll()
		{
			...
			while (subviews.Count > 0) Remove(subviews[0]);
		}
		...
		{
			ViewToScreen(rect.X, rect.Y, out var x, out var y, false);
...
//						if (view.Frame.IntersectsWith (clipRect) && view.Frame.IntersectsWith (region)) {
		...
		public bool FocusPrev()
		{
			...
			if (focused_idx != -1 && SuperView is Window) {
				FocusLast();
				return true;
			}
		...
		static List<View> TopologicalSort(HashSet<View> nodes, HashSet<(View, View)> edges)
		{
			...
			while (S.Any()) {
				...
				foreach (var e in edges.Where(e => e.Item1.Equals(n)).ToList()) {
					...
					if (edges.All(me => me.Item2.Equals(m) == false)) S.Add(m);
				}
			}
	...
	public class Window : Toplevel, IEnumerable {
		...
		void DrawFrame()
		{
			DrawFrame(new Rect(0, 0, Frame.Width, Frame.Height), padding, true);
		}
	...
	public static class Application {
		...
		static View FindDeepestView(View start, int x, int y, out int resx, out int resy)
		{
			...
			if (start.Subviews != null) {
				var count = start.Subviews.Count;
				if (count > 0) {
		...
		public static void TerminalResized()
		{
